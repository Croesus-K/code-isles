  ...
  levels: [
    ...
    },
    {
      ...
      questions: [
        ...
        {
          kind: 'output',
          prompt: '0.1 + 0.2 的真实结果是？',
          code: '0.1 + 0.2',
          options: ['0.3', '0.30000000000000004', '0.3000000000000001', '报错'],
          answerIndex: 1,
          hint: '二进制小数没法精确表示 0.1。',
          explain: '0.1 + 0.2 在 IEEE 754 双精度下不是 0.3，而是 0.30000000000000004。这是浮点数的经典坑——金额计算常用整数（分）或 decimal 模块避开。',
        },
      ],
    },
    {
    ...
    },
    {
      ...
      questions: [
        ...
        {
          kind: 'output',
          prompt: '"a,b,c".split(",") 的结果是？',
          code: '"a,b,c".split(",")',
          options: ['"a,b,c"', '["a", "b", "c"]', '["a,b,c"]', '报错'],
          answerIndex: 1,
          hint: 'split(分隔符) 按分隔符把字符串切成列表。',
          explain: 'split(",") 按逗号把字符串切成三段，得到列表 ["a", "b", "c"]。这是把一行 CSV 解析成列表的标准写法——分隔符写在括号里，列表在外面。',
        },
        {
          kind: 'output',
          prompt: '"-".join(["a", "b", "c"]) 的结果是？',
          code: '"-".join(["a", "b", "c"])',
          options: ['"abc"', '"a-b-c"', '["a", "b", "c"]', '报错'],
          answerIndex: 1,
          hint: 'join 的方向：分隔符在点前面，把列表元素串起来。',
          explain: '分隔符.join(列表) 把列表元素串成一个字符串，中间夹分隔符。"-".join(["a","b","c"]) 得到 "a-b-c"，方向和 split 相反。',
        },
        {
          kind: 'bug',
          prompt: '下面这段代码哪一行会报错？',
          code: ['1st_place = 1', 'print(1st_place)'],
          answerLine: 0,
          hint: '变量名的第一个字符能是数字吗？',
          explain: '变量名必须以字母或下划线开头，不能以数字开头。第 1 行 1st_place = 1 报错 SyntaxError: invalid decimal literal（数字开头被当成了数字字面量）。变量名命名规则：字母/下划线开头 + 字母/数字/下划线。',
        },
      ],
    },
  ],
