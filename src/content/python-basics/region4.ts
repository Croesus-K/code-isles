    ...
    // ============ 4-3 切片 + in ============
    {
      ...
      questions: [
        ...
        },
        {
          kind: 'bug',
          prompt: '下面两行里，哪一行会报错？',
          code: ['bag = ["剑", "盾", "药水"]', 'bag[1:3] = 42'],
          answerLine: 1,
          hint: '切片赋值时，等号右边必须是可迭代对象。',
          explain: '第 2 行把整数 42 赋给切片——切片赋值要求右边是可迭代对象（列表 / 元组 / 字符串之一），整数 42 不是可迭代对象，触发 TypeError。正确写法：bag[1:3] = ["弓"]。',
        },
        {
          kind: 'output',
        ...
        {
          kind: 'fill',
          prompt: '补全代码，用切片取出索引 1 到 2 之间（不含 2）的元素：',
          code: 'bag = ["剑", "盾", "药水", "弓"]\nfirst = bag[1:___]     # → ["药水"]',
          answers: ['2'],
          placeholder: '一个数字',
          hint: '「药水」索引 1，「弓」索引 2。切片 [a:b] 含头不含尾，终点写几？',
          explain: 'bag[1:2] 是切片：含头不含尾，只取索引 1 一个元素，结果 ["药水"]。要拿单元素值用 bag[1]；要拿单元素列表用 bag[1:2]。',
        },
      ],
    },
