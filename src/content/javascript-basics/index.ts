...
const j1b: LevelDef = {
  ...
  questions: [
    ...
    },
    {
      ...
      answerIndex: 1,
      hint: '/ 在 JS 里也总返回小数。',
      explain: 'JS 里 10 / 5 得 2.0（number 类型），不是 2。console.log 输出 "2"，但类型仍是小数。这点跟 Python 一致。',
    },
    {
      kind: 'fill',
...
const j1Boss: LevelDef = {
  ...
  questions: [
    ...
    },
    {
      ...
      answerLine: 2,
      hint: '注意 x 现在是字符串。',
      explain: '第 3 行：x 在第 2 行被赋值为字符串 "1"，x + 1 触发字符串拼接得 "11"——不报错但不期望。要想加法要先 Number(x) 转回数字。这是 JS 动态类型的双刃剑：变量能装不同类型，但每次用都要想清楚现在是哪个。',
    },
    {
      kind: 'output',
      prompt: '执行后输出什么？',
...
const j4c: LevelDef = {
  ...
  questions: [
    ...
    },
    {
      ...
      answerLine: 4,
      hint: 'var 是函数作用域。',
      explain: 'var 是函数作用域，不是块作用域。if (true) { var x = 2 } 里的 var x 实际上和外面的 var x 是同一个 x（只是重新赋值）。所以 f() 返回 2。JS 这块特别坑，新手建议默认用 let/const，忘掉 var。',
    },
    {
      kind: 'output',
      prompt: '执行后 c() 是？',
...
const j4d: LevelDef = {
  ...
  questions: [
    ...
    },
    {
      ...
      answerLine: 3,
      hint: 'try 块里出错没人接。',
      explain: 'try 块里 throw new TypeError("Error") 抛错，但没 catch 接住——错误会冒泡到全局，最终抛 ReferenceError: Uncaught TypeError。这是 JS 默认行为：try 包住的代码出错而没 catch，错误会传到最外层。',
    },
    {
      kind: 'output',
      prompt: '执行后 output 是？',
...
const j4Boss: LevelDef = {
  ...
  questions: [
    ...
    },
    {
      ...
      answerLine: 3,
      hint: 'try 块里出错被谁接住？',
      explain: '第 4 行不会真的「出问题」——因为 try 块里 o.age.toUpperCase() 抛错被 catch 接住了。但 o.age 是 undefined，调 .toUpperCase() 抛 TypeError，catch 接住后打印 "caught"。这是 try/catch 保护错误的标准用法。',
    },
    {
      kind: 'output',
      prompt: '执行后 output 是？',
