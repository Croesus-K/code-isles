...
M0~M5 已完成：像素设计系统、存档系统、世界地图与关卡流程、五种题型与经济闭环（星级/连击/提示商店）、Python 基础全课程（5 区域 20 关 135 题）、成就系统（10 枚徽章 + 结算弹窗 + 徽章墙）、零素材 8-bit 音效（Web Audio 合成）、结算/星级/连击/解锁的像素风动画（尊重 prefers-reduced-motion）、打赏入口（游戏内弹窗，赞助页接入待定）、部署上线（博客 Cloudflare Pages 静态托管）。里程碑规划见 [docs/product-plan.md](docs/product-plan.md)。
